#include <iostream>   //input |outout
#include <string>    //for string understanding
#include <iomanip>   //input,output and goto
...
class MenuItem {
    ...
    public:
        ...
        MenuItem(string n, string desc, double p) : name(n), description(desc), price(p) {}  //parameterized constructors
...
class Table {
    ...
    Table(int number) : tableNumber(number), occupied(false), reserved(false) {} //parameterized constructors
    ...
    void vacate() {
        ...
        occupied = false;
        reserved = false;
        order = Order(); // Clear the order when vacating the table
...
public:
    ...
    User(string uname, string pass) : username(uname), password(pass) {}   //parameterized constructors
    virtual ~User() {}
...
public:
    ...
    Waiter(string uname, string pass) : User(uname, pass) {}   //parameterized constructors
        ...
        do {
            ...
            switch (choice) {
                ...
                case 2: {
                    ...
                    menu.display(); // Display the menu when taking an order
                ...
                case 3: {
                    ...
                    table.displayOrder();
                    double bill = table.getBill(0.10); // Assume a 10% tax rate
    ...
    void menuActions(Menu& menu, Table tables[], int& tableCount) override {
        ...
        switch (choice) {
            ...
            case 2: {
                ...
                table = Table(tableNumber); // Initialize the table with the given number
                table.occupy();
...
int main() {
    ...
    for (int i = 0; i < MAX_TABLES; ++i) {
        tables[i] = Table(i + 1); // Initialize each table with its number
    }
    
    int tableCount = MAX_TABLES; // Assume all tables are available initially
    ...
    do {
        ...
        // After exiting the user type selection loop, prompt to exit or continue
